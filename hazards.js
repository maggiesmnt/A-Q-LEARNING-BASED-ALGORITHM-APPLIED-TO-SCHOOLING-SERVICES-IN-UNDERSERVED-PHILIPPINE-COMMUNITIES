...
function renderHazards(){
  ...
  all.forEach(function(r){
    ...
    var conf=confidence(r),A=accA(e),b=band(A);
    ...
    d.innerHTML='<div class="row"><div style="font-size:22px">'+r.em+'</div><div style="flex:1">'+
      ...
      '<div class="k">'+N[e.a].name+' &harr; '+N[e.b].name+' &middot; '+SURF[e.surf].lab+'</div>'+
      '<div class="k">Accessibility basis: '+accessibilityReason(e)+'</div></div>'+
      ...
      '<div class="meter"><i style="width:'+(conf*100).toFixed(0)+'%;background:'+(conf>0.6?"var(--bad)":conf>0.3?"var(--warn)":"var(--dim2)")+'"></i></div>'+
      '<div class="k">Source: '+r.who+' &middot; confidence <b>'+(conf*100).toFixed(0)+'%</b></div>'+
